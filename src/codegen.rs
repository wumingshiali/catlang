...
impl ZigCodeGen {
    ...
    pub fn with_sandbox(sandbox: Option<SandboxConfig>) -> Self {
        Self {
    ...
    pub fn generate(mut self, program: &Program) -> CodeGenResult<String> {
        ...
        // Generate sandbox initialization if enabled
        if let Some(ref sandbox) = self.sandbox {
            let sandbox_code = sandbox.to_zig_init_code();
            if !sandbox_code.is_empty() {
                self.writeln(&sandbox_code.trim_end());
                self.writeln("");
            }
        }
    ...
    fn generate_needed_helpers(&mut self) {
        ...
        // Generate sandbox file I/O helpers if sandbox is enabled
        if let Some(ref sandbox) = self.sandbox {
            if sandbox.enabled {
                self.generate_sandbox_helpers();
            }
        }
    }

    fn generate_sandbox_helpers(&mut self) {
        self.writeln("// Sandbox file access helpers");
        self.writeln("// Checks if a path is allowed by the sandbox policy");
        self.writeln("fn isPathAllowed(path: []const u8) bool {");
        self.indent_level += 1;
        self.writeln("if (!SANDBOX_ENABLED) return true;");
        self.writeln("");
        self.writeln("var resolved_path_buf: [4096]u8 = undefined;");
        self.writeln("const resolved = std.fs.cwd().realpath(path, &resolved_path_buf) catch return false;");
        self.writeln("");
        self.writeln("var i: usize = 0;");
        self.writeln("while (i < sandbox_allowed_paths.len) : (i += 1) {");
        self.indent_level += 1;
        self.writeln("if (std.mem.startsWith(u8, resolved, sandbox_allowed_paths[i])) {");
        self.indent_level += 1;
        ...
        self.writeln("}");
        self.writeln("");
        self.writeln("std.debug.print(\"[sandbox] Access denied: {s}\\n\", .{path});");
        self.writeln("return false;");
        self.indent_level -= 1;
        self.writeln("}");
        self.writeln("");

        self.writeln("// sandboxOpenFile - open a file with sandbox check");
        self.writeln("fn sandboxOpenFile(path: []const u8, mode: std.fs.File.OpenFlags) ?std.fs.File {");
        self.indent_level += 1;
        self.writeln("if (!isPathAllowed(path)) return null;");
        self.writeln("return std.fs.cwd().openFile(path, mode) catch |err| {");
        self.indent_level += 1;
        self.writeln("std.debug.print(\"[sandbox] Cannot open file: {s} (error: {})\\n\", .{ path, err });");
        ...
        self.writeln("// sandboxCreateFile - create a new file with sandbox check");
        self.writeln("fn sandboxCreateFile(path: []const u8) ?std.fs.File {");
        self.indent_level += 1;
        self.writeln("if (!isPathAllowed(path)) return null;");
        self.writeln("return std.fs.cwd().createFile(path, .{}) catch |err| {");
        self.indent_level += 1;
        self.writeln("std.debug.print(\"[sandbox] Cannot create file: {s} (error: {})\\n\", .{ path, err });");
        self.writeln("return null;");
        self.indent_level -= 1;
        self.writeln("};");
        self.indent_level -= 1;
        ...
        self.writeln("// sandboxReadFile - read entire file with sandbox check");
        self.writeln("fn sandboxReadFile(path: []const u8, allocator: std.mem.Allocator) ?[]u8 {");
        self.indent_level += 1;
        self.writeln("if (!isPathAllowed(path)) return null;");
        self.writeln("return std.fs.cwd().readFileAlloc(allocator, path, 1048576) catch |err| {");
        self.indent_level += 1;
        self.writeln("std.debug.print(\"[sandbox] Cannot read file: {s} (error: {})\\n\", .{ path, err });");
        self.writeln("return null;");
        self.indent_level -= 1;
        self.writeln("};");
        self.indent_level -= 1;
        ...
        self.writeln("// sandboxWriteFile - write to file with sandbox check");
        self.writeln("fn sandboxWriteFile(path: []const u8, data: []const u8) bool {");
        self.indent_level += 1;
        self.writeln("if (!isPathAllowed(path)) return false;");
        self.writeln("var file = std.fs.cwd().createFile(path, .{}) catch |err| {");
        self.indent_level += 1;
        self.writeln("std.debug.print(\"[sandbox] Cannot write file: {s} (error: {})\\n\", .{ path, err });");
        self.writeln("return false;");
        self.indent_level -= 1;
        self.writeln("};");
        self.writeln("defer file.close();");
        self.writeln("file.writeAll(data) catch |err| {");
        self.indent_level += 1;
        self.writeln("std.debug.print(\"[sandbox] Cannot write to file: {s} (error: {})\\n\", .{ path, err });");
        self.writeln("return false;");
        self.indent_level -= 1;
        self.writeln("};");
        self.writeln("return true;");
        ...
        self.writeln("// sandboxDeleteFile - delete a file with sandbox check");
        self.writeln("fn sandboxDeleteFile(path: []const u8) bool {");
        self.indent_level += 1;
        self.writeln("if (!isPathAllowed(path)) return false;");
        self.writeln("std.fs.cwd().deleteFile(path) catch |err| {");
        self.indent_level += 1;
        self.writeln("std.debug.print(\"[sandbox] Cannot delete file: {s} (error: {})\\n\", .{ path, err });");
        self.writeln("return false;");
        self.indent_level -= 1;
        self.writeln("};");
        self.writeln("return true;");
...
/// Public function to generate Zig code from AST with sandbox configuration
pub fn generate_zig_with_sandbox(program: &Program, sandbox: Option<SandboxConfig>) -> CodeGenResult<String> {
    ZigCodeGen::with_sandbox(sandbox).generate(program)
}
